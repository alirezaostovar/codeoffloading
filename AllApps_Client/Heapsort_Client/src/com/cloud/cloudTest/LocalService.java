...
//    SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(context);
  ...
  private void RunFuncs() {
      ...
      double avgHeapsortTime = 0;
      for (int i = 0; i < 4; i++){
    	  long internalstTime = System.nanoTime();

    	  Heapsort pf = new Heapsort();
          pf.localRunHeapsort();

          execTimes.add((System.nanoTime() - internalstTime)*1.0e-6);
      }
      avgHeapsortTime = avg(execTimes);



      notifytochangelable("Average Heapsort Time: " + avgHeapsortTime);
  }
  ...
  public void notifytochangelable(String HeapsortTime){
		Intent intent = new Intent(MainActivity.changelabel);
	    intent.putExtra("iscloud", false);
	    intent.putExtra("Heapsortintent", HeapsortTime);
