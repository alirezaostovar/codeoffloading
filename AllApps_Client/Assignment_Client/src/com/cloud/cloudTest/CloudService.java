...
import AssignmentTest.Assignment;
...
//    SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(context);
  ...
  private void RunFuncs() {
	...
	//    Assignment
	   ...
	   double avgAssignmentTime = 0;
	   for (int i = 0; i < 4; i++){
	       long internalstTime = System.nanoTime();
	
	       Assignment nq = new Assignment();
	       nq.dotest();
	
	       execTimes.add((System.nanoTime() - internalstTime)*1.0e-6);
	   }
	   avgAssignmentTime = avg(execTimes);


      notifytochangelable("Average Assignment Time: " + avgAssignmentTime);
  }
  ...
  public void notifytochangelable(String AssignmentTime){
		Intent intent = new Intent(MainActivity.changelabel);
	    intent.putExtra("iscloud", true);
	    intent.putExtra("Assignmentintent", AssignmentTime);
