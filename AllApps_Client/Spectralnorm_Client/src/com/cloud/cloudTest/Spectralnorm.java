...
public class Spectralnorm
 extends CloudRemotable {
    ...
    @Cloud
    public void localRunSpectralnorm() {
        int n = 1000;
    ...
    public void RunSpectralnorm() {
	Method toExecute; 
	Class<?>[] paramTypes = null; 
	Object[] paramValues = null; 
	
	try{
		toExecute = this.getClass().getDeclaredMethod("localRunSpectralnorm", paramTypes);
		Vector results = getCloudController().execute(toExecute,paramValues,this,this.getClass());
		if(results != null){
			copyState(results.get(1));
		}else{
			localRunSpectralnorm();
		}
