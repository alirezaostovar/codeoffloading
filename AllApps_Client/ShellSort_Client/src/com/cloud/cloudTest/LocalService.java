...
//    SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(context);
  ...
  private void RunFuncs() {
      ...
      double avgShellSortTime = 0;
      for (int i = 0; i < 4; i++){
    	  long internalstTime = System.nanoTime();

    	  ShellSort pf = new ShellSort();
          pf.localRunShellSort();

          execTimes.add((System.nanoTime() - internalstTime)*1.0e-6);
      }
      avgShellSortTime = avg(execTimes);



      notifytochangelable("Average ShellSort Time: " + avgShellSortTime);
  }
  ...
  public void notifytochangelable(String ShellSortTime){
		Intent intent = new Intent(MainActivity.changelabel);
	    intent.putExtra("iscloud", false);
	    intent.putExtra("ShellSortintent", ShellSortTime);
