...
//    SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(context);
  ...
  private void RunFuncs() {
      ...
      double avgFastareduxTime = 0;
      for (int i = 0; i < 4; i++){
    	  long internalstTime = System.nanoTime();

    	  Fastaredux pf = new Fastaredux();
    	  try {
				pf.localRunFastaredux();
			} catch (IOException e) {
      ...
      avgFastareduxTime = avg(execTimes);



      notifytochangelable("Average Fastaredux Time: " + avgFastareduxTime);
  }
  ...
  public void notifytochangelable(String FastareduxTime){
		Intent intent = new Intent(MainActivity.changelabel);
	    intent.putExtra("iscloud", false);
	    intent.putExtra("Fastareduxintent", FastareduxTime);
