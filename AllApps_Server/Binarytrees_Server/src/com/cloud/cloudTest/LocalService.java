...
//    SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(context);
  ...
  private void RunFuncs() {
      ...
      double avgBinarytreesTime = 0;
      for (int i = 0; i < 4; i++){
    	  long internalstTime = System.nanoTime();

    	  Binarytrees pf = new Binarytrees();
          pf.localRunbinarytrees();

          execTimes.add((System.nanoTime() - internalstTime)*1.0e-6);
      }
      avgBinarytreesTime = avg(execTimes);



      notifytochangelable("Average Binarytrees Time: " + avgBinarytreesTime);
  }
  ...
  public void notifytochangelable(String BinarytreesTime){
		Intent intent = new Intent(MainActivity.changelabel);
	    intent.putExtra("iscloud", false);
	    intent.putExtra("Binarytreesintent", BinarytreesTime);
