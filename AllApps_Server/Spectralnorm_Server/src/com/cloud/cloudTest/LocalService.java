...
//    SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(context);
  ...
  private void RunFuncs() {
      ...
      double avgSpectralnormTime = 0;
      for (int i = 0; i < 4; i++){
    	  long internalstTime = System.nanoTime();

    	  Spectralnorm pf = new Spectralnorm();
          pf.localRunSpectralnorm();

          execTimes.add((System.nanoTime() - internalstTime)*1.0e-6);
      }
      avgSpectralnormTime = avg(execTimes);



      notifytochangelable("Average Spectralnorm Time: " + avgSpectralnormTime);
  }
  ...
  public void notifytochangelable(String SpectralnormTime){
		Intent intent = new Intent(MainActivity.changelabel);
	    intent.putExtra("iscloud", false);
	    intent.putExtra("Spectralnormintent", SpectralnormTime);
